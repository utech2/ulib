...
class CustomSearchView extends StatefulWidget {
  final Future<QuerySnapshot> queri; // Correct the type to Future<QuerySnapshot>

  const CustomSearchView({super.key, required this.queri});
...
class _CustomSearchViewState extends State<CustomSearchView> {
  ...
  Future<void> _loadAllNotes() async {
    try {
      QuerySnapshot querySnapshot = await widget.queri; // Correctly await the Future<QuerySnapshot>
  ...
  void _loadBannerAd() {
    _bannerAd = BannerAd(
      adUnitId: 'ca-app-pub-8354235211065834/9812044905', // Test Ad Unit ID
      size: AdSize.banner,
      request: AdRequest(),
